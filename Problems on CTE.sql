...
select EmployeeID, OrderID, OrderDate, LastDayOfMonth
from 
	  (
      Select EmployeeID, OrderID, OrderDate, MONTH(OrderDate) OrderMonth, YEAR(OrderDate) OrderYear,
	  LAST_VALUE(OrderDate) over(partition by  MONTH(OrderDate), YEAR(OrderDate) order by OrderDate range between unbounded preceding and unbounded following) as LastDayOfMonth
      from [dbo].[Orders]
	  ) as OrderData
...
-- Sol : Orders Table are Used
select OrderID, convert(Date,OrderDate) as OrderDate, convert(Date,RequiredDate) as RequiredDate, convert(Date,ShippedDate) as ShippedDate
from Orders
